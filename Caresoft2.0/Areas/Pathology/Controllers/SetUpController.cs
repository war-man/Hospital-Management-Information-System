using Caresoft2._0.Areas.Pathology.Models;
...
namespace Caresoft2._0.Areas.Pathology.Controllers
{
    ...
    public class SetUpController : Controller
    {
        ...
        {
            PathologyAddNewUserData pathologyAddNewUserData = new PathologyAddNewUserData();
            pathologyAddNewUserData.Department = db.Departments.Where(d => d.Id.Equals(main_department_id)).ToList();
            pathologyAddNewUserData.MainDepartments = dblabs.Departments.Where(e => e.DepartmentRadPath.Equals(main_department_id)).ToList();
            pathologyAddNewUserData.UserType = dblabs.UserTypes.ToList();
            ...
            pathologyAddNewUserData.Employees = db.Employees.Where(c => !existingUserIds.Contains(c.Id) && c.DepartmentId == main_department_id).ToList();


            return View(pathologyAddNewUserData);
        }
        ...
        public async Task<int> AssignNewDepartment(DepartmentAssignmentModel newUserData)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (da == null)
                {
                    bool checkUsernameExists = dblabs.DepartmentAssignments.Any(e => e.DepartmentRadPath == main_department_id && e.UserName.Equals(local_emp.FName + "" + local_emp.OtherName));
        ...
        private int UploadSignature(HttpPostedFileBase SingatureImage, int User)
        {
            if (SingatureImage != null)
            {
                ...
                string directory = "~/Areas/Pathology/Images/Signatures";
        ...
        public async Task<ActionResult> Edit(int Id)
        {
            ...
            PathologyAddNewUserData pathologyAddNewUserData = new PathologyAddNewUserData();
            pathologyAddNewUserData.Department = db.Departments.Where(d => d.DepartmentName == "Pathology").ToList();
            pathologyAddNewUserData.MainDepartments = dblabs.Departments.Where(e => e.DepartmentRadPath == main_department_id).ToList();
            pathologyAddNewUserData.UserDepartmentAssignment = user;

            pathologyAddNewUserData.Employees = db.Employees.Where(e => e.Id == user.User).ToList();
            pathologyAddNewUserData.UserType = dblabs.UserTypes.ToList();
        ...
        public async Task<ActionResult> TestMaster(int? Departments, string TestCode, string TestName)
        {
            ...
            {
                ViewBag.Layout = "~/Areas/Pathology/Views/Shared/_LayoutPathology.cshtml";
            }
        ...
        public ActionResult MarqueeMaster(PathKeyValuePair marquee)
        {
            ...
            if (ModelState.IsValid)
            {
                var marq = dblabs.PathKeyValuePairs.FirstOrDefault(e => e.Key_.Equals("PathologyDepartmentMarquee"));
                ...
                {
                    marquee.Key_ = "PathologyDepartmentMarquee";

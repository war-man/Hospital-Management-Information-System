...
namespace Caresoft2._0.Areas.Pathology.Controllers
{
    ...
    public class TestResultsEntryController : Controller
    {
        ...
        // GET: Pathology/TestResultsEntry
        ...
        public async Task<ActionResult> AccesionList(TestEntryPatients filter, int? page)
        {
            ...
            e.ShowInSpecimentResultEnty == true && e.WorkOrderTests.Any(f => f.LabTest.DepartmentRadPath == main_department_id));
        ...
        public JsonResult searchPatientName(string search)
        {
            ...
            var patientOpd = db2.Patients.Where(e => e.RegNumber.Contains(search) || e.FName.Contains(search) || e.MName.Contains(search) ||
        ...
        public async Task<int> ResultsEntry(FormCollection testEntry)
        {
            if (ModelState.IsValid)
            {
                ...
                for (int i = 0; i < testEntry.Count; i++)
                {
                    ...
                    if (testEntry.GetKey(i).StartsWith("Auth"))
                    {
                        ...
                        int res = await AutorizeTestResults(testId);
        ...
        public async Task<int> AutorizeTestResults(int id)
        {
            if (ModelState.IsValid)
            {
